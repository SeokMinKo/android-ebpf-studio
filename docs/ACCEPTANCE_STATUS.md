...
Installed-native checks have partial independent coordinate/bin/filter/CSV coverage. Full graph, theme and interaction acceptance is still pending. The original Compare area QA rejected a valid dataset-dependent empty rectangle. This harness failure is superseded by the later independent empty/populated rectangle checks documented in [Compare area QA](COMPARE_AREA_QA.md). Earlier single-file capture runtime misses remain recorded; a later zero-miss capture does not erase them.
...
## Installed Compare acceptance increment (2026-09-08)

The backed-up installed Windows EXE passed 30 shared/local filter runs across light/dark/contrast and an additional 30 actual Compare export button runs. Each batch independently matched 60 baseline/current populations to raw NDJSON: full request keys, Read/Write bytes, time origin and Time(ms)/Address(MB) coordinates. Saved export files also matched file candidate paths, counts, timing percentiles and selected-span throughput. Three screenshots per batch were directly inspected; the other recorded screenshots are not claimed as a complete visual-quality pass. QA supplies filter values and the export filename; input typing and the OS save dialog are excluded.

Local validation: 260 host tests passed, 13 ignored; Clippy, rustfmt and release build passed. Both source session hashes stayed unchanged. Empty/populated Compare rectangles and settled latency percentiles also have scoped native evidence. Physical Exact success and the full visible-graph interaction/downsampling matrix remain incomplete.
