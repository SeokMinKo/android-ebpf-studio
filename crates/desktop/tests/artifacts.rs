...
fn release_bundle_artifacts_and_session_path_are_automatic() {
    let root = std::env::temp_dir().join(format!("android-ebpf-studio-paths-{}", std::process::id()));
    ...
    let paths = CapturePaths::discover_from(&bundle.join("android-ebpf-studio.exe"), &root, &sessions).unwrap();
    ...
    assert_eq!(paths.session.extension().and_then(|value| value.to_str()), Some("ndjson"));
    fs::remove_dir_all(root).ok();
}

#[test]
fn missing_agent_is_actionable() {
    let root = std::env::temp_dir().join(format!("android-ebpf-studio-missing-{}", std::process::id()));
    fs::create_dir_all(&root).unwrap();
    let result = CapturePaths::discover_from(&root.join("studio.exe"), &root, &root.join("sessions"));
